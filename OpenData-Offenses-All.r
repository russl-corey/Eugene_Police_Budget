...
setwd("~/Documents/Oregon_Crime_Report")
...
county_list <- sort(unique(tolower(offenses$County)))
...
offenses$agency <- tolower(offenses$`Agency Name`)

# Encode violent offenses vo = 1 for violent assault, 0 for not
violent_titles <- c("Aggravated Assault",
                    ...
                    'Willful Murder')
offenses$vo <- 0
offenses$vo[offenses$`NIBRS Report Title` %in% violent_titles] <- 1
